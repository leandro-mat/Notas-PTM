...
Uma variável aleatória $X$ em $(\Omega,\mathcal{F})$ (v.a.) é 
...
\begin{observacao}
A definição nesta generalidade é necessária por razões
técnicas em algumas aplicações, mas para a discussão das
...
assim ao invés de escrever $X:\Omega\to \overline{\R}$ é uma 
variável aleatória, vamos escrever simplesmente $X$ é uma v.a..
...
Agora fixamos $A_1\in \sigma(\mathcal{C}_1)$.
De maneira análoga definimos 
	\[
	\mathcal{K}=
	\{ A\in \F: \P(A\cap A_1) = \P(A)\P(A_1)\}
	\]
e podemos verificar que está coleção é um $\lambda$-sistema.
...
Também são apresentados nesta seção alguns critérios para
independência de v.a.'s.

\begin{definicao}[$\sigma$-álgebra Induzida por uma v.a.]
Sejam $(\Omega,\F)$ 
um espaço de mensurável e $X$ uma v.a. definida $\Omega$.
A $\sigma$-álgebra induzida por $X$, 
notação, $\sigma(X)$ é definida como sendo a 
sub-$\sigma$-álgebra de $\F$ dada por 
\[
	\sigma(X)
	:= 
	\{
		X^{-1}(B): B\in\mathscr{B}(\overline{\mathbb{R}})
	\}.
\]
\end{definicao}
...
deste conceito é dado por uma coleção 
de funções indicadoras de determinados eventos. 
Primeiro observe, para qualquer evento $A$, que 
	\[
...
$\{\mathcal{C}_j\}_{j\in J}$ é uma coleção de eventos
...
\begin{corolario}
	...
	independente se, e somente se, 
		\[
			\P \left(\, \bigcap_{j=1}^n \{X_j\leq x_j\} \right)
			\equiv
...
e $X_1,\ldots,X_n$ uma coleção de 
...
temos
	...
	\sup_{k\in\Z}
		...
		\right).
	\end{align*}
 Usando novamente que $F$ é uma função distribuição 
 e contínua podemos
...
$(\Omega,\F,\P)$. 
Dizemos que há um ``empate'' entre estas v.a.'s  em 
$\omega\in\Omega$ se temos que
...
Vamos usar a notação simplificada 
	\[
		...
		\bigcup_{i\neq j} \{X_i=X_j\}.
	\]
Usando o lema anterior e a subaditividade de $\mathbb{P}$ 
podemos ver imediatamente que
$\P(\{\text{Empate}\})=0$.
Fixado $\omega\in\Omega$ vamos dizer 
que $X_n(\omega)$ 
é um recorde (entre $X_1(\omega),\ldots,X_{n-1}(\omega)$)
se 
	\[ 
	X_n(\omega)>\max\{X_1(\omega),\ldots,X_{n-1}(\omega)\}.
	\]
Para facilitar vamos denotar 
o evento 
\[
\{\omega\in\Omega: X_n(\omega)>\max\{X_1(\omega),\ldots,X_{n-1}(\omega)\} \} 
\]
por $\{X_n \ \text{é um recorde}\ \}$ ou simplesmente $A_n$.
...
os eventos $\{A_n\}$ são independentes e também que 
	\[
		...
		\forall\ j\geq 2.
	\]
De maneira mais geral, podemos definir o ranque
relativo de $X_n$ que é v.a. dada por
	\[
