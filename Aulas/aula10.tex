...
uma função $\mathscr{X}\times \mathscr{Y}$-mensurável 
\textbf{não-negativa}. Nestas condições valem (\ref{Fubini1}) e (\ref{Fubini2}). 
\end{teorema}
...
Para isso vamos calcular separadamente os lados direito e esquerdo de
...
Temos diretamente da definição da integral de Lebesgue que
\begin{equation}
\int_X
...
concluímos que as igualdades (\ref{Fubini1}) e (\ref{Fubini2})  
também são válidas para quaisquer funções simples desde que 
...
Já que estamos assumindo que $f$ é não-negativa, 
esta aproximação pode ser feita 
via o Teorema \eqref{teo:aproximacao-monotona-por-func-simples}.
Por último, usamos o Teorema de Convergência Monótona 
para verificar que (\ref{Fubini1}) e (\ref{Fubini2}) 
valem para qualquer função $f$ $\mathscr{X}\times\mathscr{Y}$-mensurável
não-negativa.
\end{proof}
