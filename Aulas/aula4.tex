...
\begin{proof}
	...
	Passamos agora para a prova do item 3.  Considere uma subsequência
	 $(x_{n_j})\subset (x_n)$ monótona 	crescente  com
	  ...
	  \]
segue que dado  $\epsilon>0$ existe $n_{j_0}$ tal que para todo $n_j>n_{j_0}$ 
vale que $F(x_{n_j})>1-\epsilon$. Como $x_n\to \infty$ temos q
ue existe $N$ tal que para todo $n>N$ tem-se $x_n> x_{n_{j_0}}$
